...
class Bind_EweiShopV2Model
{
	...
	//统计下线人数，每推广30人统计一次，下线无订单，冻结代理商资格
    public function checkAgents(){
		global $_W;
		$openid = $_W['openid'];
		$member = m('member')->getMember($openid);
		if(!empty($member['agentid'])){
            $memberagent = m('member')->getMember($member['agentid']);
            if(!empty($memberagent['status'])){
                m('member')->setCredit($memberagent['openid'], 'credit2', 1, '推广用户奖励余额 +1 openid:'.$openid);
                ...
                if($hadextend==0){
                    ...
                    if(empty($ordercount)){
                        pdo_update('ewei_shop_member',array('status'=>0),array('uniacid'=>$_W['uniacid'], 'openid'=>$memberagent['openid']));
                        //发送通知，代理资格已经被冻结
                        $message = '你最近推广的30位用户都没有下单，你已被限制推广！请提醒最近30位客户付邮体验产品，便可再次获取推广资格！';
