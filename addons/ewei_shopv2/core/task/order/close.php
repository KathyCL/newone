...
foreach ($sets as $set) {
	...
	if (isset($trade['closeorder_virtual'])) {
	...
	if (0 < $min) {
		$mintimes = 60 * $min;
		$orders = pdo_fetchall('select id,openid,deductcredit2,ordersn,isparent,deductcredit,deductprice,status,isparent,isverify,`virtual`,`virtual_info` from ' . tablename('ewei_shop_order') . ' where uniacid=' . $_GPC['uniacid'] . '  and paytype<>3  and `virtual` <> 0 and ((createtime + ' . $mintimes . ' <=unix_timestamp() and status=0) or (status = 1 and `isverify` = 1 and `verifyendtime` <= unix_timestamp() and `verifyendtime` > 0))');
		$p = com('coupon');
