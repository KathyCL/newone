...
if (!class_exists('CommissionModel')) {
	class CommissionModel extends PluginModel
	{
				...
				foreach ($goods as &$cinfo) {
					...
					if ($update) {
						...
						if (!empty($agentid)) {
							...
							$m1memberlevel = m('member')->getLevel($agentid);  //wwz 会员等级
							$agentDiscountPrice = m('order')->getGoodsDiscountPrice($cinfo,$m1memberlevel);
                            $memberlevel = m('member')->getLevel($_W['openid']);  //wwz 会员等级
                            $memberDiscountPrice = m('order')->getGoodsDiscountPrice($cinfo,$memberlevel);


                            WeUtility::logging('$m1memberlevel',$m1memberlevel);
							WeUtility::logging('$agentDiscountPrice',$agentDiscountPrice);
                            WeUtility::logging('$memberlevel',$memberlevel);
                            WeUtility::logging('$memberDiscountPrice',$memberDiscountPrice);
							if (($m1['isagent'] == 1) && ($m1['status'] == 1)) {
								...
								//wwz 差额佣金
								if(!empty($memberDiscountPrice['price2'])||!empty($agentDiscountPrice['price2'])){
                                    $ceyj = $memberDiscountPrice['price2'] - $agentDiscountPrice['price2'];
								}else{
                                    $ceyj = $cinfo['realprice']-$cinfo['price']*$m1memberlevel['discount']/10;
                                    if($ceyj<0){
                                        $ceyj = 0;
                                    }
								}
						...
						pdo_update('ewei_shop_order_goods', array('ceyj' => $ceyj,'commission1' => iserializer($cinfo['commission1']), 'commission2' => iserializer($cinfo['commission2']), 'commission3' => iserializer($cinfo['commission3']), 'commissions' => iserializer($commissions), 'nocommission' => $cinfo['nocommission']), array('id' => $cinfo['id']));
					}
				}
		...
		public function getInfo($openid, $options = NULL)
		{
			...
			if (1 <= $level) {
				...
				if (in_array('total', $options)) {
					$level1_commissions = pdo_fetchall('select og.commission1,og.commissions,og.ceyj  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
				...
				if (in_array('ok', $options)) {
					$level1_commissions = pdo_fetchall('select og.commission1,og.commissions,og.ceyj  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=3 and og.nocommission=0 and (' . $time . ' - o.finishtime >= ' . $day_times . ') and og.status1=0  and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
					foreach ($level1_commissions as $c) {
						$commissions = iunserializer($c['commissions']);
						$commission = iunserializer($c['commission1']);
				...
				if (in_array('lock', $options)) {
					$level1_commissions1 = pdo_fetchall('select og.commission1,og.commissions,og.ceyj  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=3 and og.nocommission=0 and (' . $time . ' - o.finishtime <= ' . $day_times . ')  and og.status1=0  and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
				...
				if (in_array('apply', $options)) {
					$level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions,og.ceyj  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=3 and og.status1=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
				...
				if (in_array('check', $options)) {
					$level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions,og.ceyj  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=3 and og.status1=2 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
				...
				if (in_array('pay', $options)) {
					$level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions,og.ceyj  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=3 and og.status1=3 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
				...
				if (in_array('wait', $options)) {
					$level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions,og.ceyj  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and (o.status=2 or o.status=1) and og.status1=0  and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
		...
		public function checkAgent($openid = '')
		{
			...
			//wwz $parent_is_agent = !empty($parent) && ($parent['isagent'] == 1) && ($parent['status'] == 1);
            $parent_is_agent = !empty($parent) && ($parent['isagent'] == 1);
		...
		public function checkOrderConfirm($orderid = '0')
		{
			...
			$agentmember = m('member')->getMember($agentid);  //wwz
			if(!empty($agentmember['status'])){  //wwz
                if (!empty($agentid)) {
					...
					if (empty($member['agentid']) && ($member['id'] != $parent['id'])) {
						if (empty($member['fixagentid'])) {
							...
							if (empty($order['agentid'])) {
								...
								if(!empty($parent['status'])){   //wwz add
		...
		public function checkOrderFinish($orderid = '')
		{
			...
			if (!empty($member['agentid'])) {
				...
				if (!empty($parent) && ($parent['isagent'] == 1) && ($parent['status'] == 1)) {
					...
					if(empty($parent['agentlevel'])){
                        //wwz 发送通知 推广佣金达到300元后限制推广
                        $parentinfo = $this->getInfo($parent['openid'],array('ok'));
                        if($parentinfo['commission_ok']>300){
                            pdo_update('ewei_shop_member',array('status'=>0),array('uniacid'=>$_W['uniacid'], 'openid'=>$parentinfo['openid']));

                            $message = '你已经获取300元推广佣金，即将被限制推广，仅仅在平台购物满199元，即可获取永久推广资格，同时获取5折起优惠特权！';
                            m('notice')->sendCheckAgentMessage($message,$parentinfo['openid']);
                        }
					}
