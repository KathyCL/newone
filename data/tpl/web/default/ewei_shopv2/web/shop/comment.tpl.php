...
<table class="table table-hover table-responsive">
    ...
    <tr>
        ...
        <td>
            <a class='btn btn-default btn-sm'  href="<?php  echo webUrl('shop/comment/post', array('id' => $row['id'], 'type' => 1))?>" title='进行回复'>审核</a>

            <?php  if(!empty($row['openid'])) { ?>
                <?php if(cv('shop.comment.post')) { ?>
                <a class='btn btn-default btn-sm'  href="<?php  echo webUrl('shop/comment/post', array('id' => $row['id']))?>" title='进行回复'><i class="fa fa-reply"></i> 回复</a>
                <?php  } ?>
            <?php  } else { ?>
                <?php if(cv('shop.comment.edit')) { ?>
                <a class='btn btn-default btn-sm'  href="<?php  echo webUrl('shop/comment/edit', array( 'id' => $row['id']))?>" title='修改评价'><i class="fa fa-edit"></i> 编辑</a>
                <?php  } ?>
            <?php  } ?>

            <?php if(cv('shop.comment.add')) { ?>
            <a class='btn btn-default  btn-sm'  href="<?php  echo webUrl('shop/comment/add', array( 'goodsid' => $row['goodsid']))?>" title='添加此商品评价'><i class="fa fa-plus"></i> 添加</a>
            <?php  } ?>
            <?php if(cv('shop.comment.delete')) { ?>
            <a class='btn btn-default  btn-sm'  data-toggle='ajaxRemove'   href="<?php  echo webUrl('shop/comment/delete', array('id' => $row['id']))?>" data-confirm="确认删除此评价吗？"><i class="fa fa-trash"></i> 删除</a>
            <?php  } ?>
        </td>
    </tr>
    <?php  } } ?>
